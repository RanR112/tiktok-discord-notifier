...
- GitHub **menonaktifkan scheduled workflow otomatis** kalau repo tidak ada aktivitas (commit apa pun) selama 60 hari. **Tidak perlu tindakan manual** untuk ini: `lastCheckedAt` di `data/state.json` berubah setiap siklus, jadi workflow ini **selalu menghasilkan commit baru setiap kali jalan** (tiap 5 menit) dan commit itu terhitung sebagai aktivitas — repo otomatis tetap "aktif" dengan sendirinya.
